...
static lathe_update_status_t published;
static atomic_bool requested, enabled, uploading, reconfigure, initialized;
bool lathe_network_initialized(void) { return atomic_load(&initialized); }
...
static void message(const char *text)
{
    ...
    lathe_critical_enter(&lock, 4000 + __LINE__);
    memcpy(published.message, next, sizeof(next));
    lathe_critical_exit(&lock);
}
bool lathe_update_active(void) { return atomic_load(&enabled) || validation_pending; }
...
void lathe_update_cancel(void)
{
    ...
    if (!atomic_load(&uploading)) {
        ...
        if (!validation_pending)
            lathe_operation_release(LATHE_OWNER_UPDATE);
    }
    lathe_critical_exit(&lock);
}
void lathe_update_snapshot(lathe_update_status_t *s)
{
    lathe_critical_enter(&lock, 4000 + __LINE__);
    *s = published;
    lathe_critical_exit(&lock);
    s->active = lathe_update_active();
    s->pairing_required = LATHE_OTA_REQUIRE_PAIRING;
...
static void transfer(int fd)
{
    ...
    if (LATHE_OTA_REQUIRE_PAIRING) {
        char greeting[73] = "P4OTA1 ";
        ...
        send(fd, "P4OTA0\n", 7, 0);
    if (!receive(fd, header, LATHE_OTA_REQUIRE_PAIRING ? sizeof(header) : 36))
        return;
    if (LATHE_OTA_REQUIRE_PAIRING) {
    ...
    message(LATHE_OTA_REQUIRE_PAIRING ? "Receiving authenticated firmware" : "Receiving firmware (LAN pairing disabled)");
    ...
    while (received < size) {
        ...
        if (!received) {
            ...
            if (buffer[0] != ESP_IMAGE_HEADER_MAGIC ||
                !lathe_ota_project_allowed(description->project_name)) {
...
static void network_task(void *arg)
{
    ...
    for (;;) {
        ...
        if (started && (uint32_t)(esp_timer_get_time() / 1000) - last_retry > 10000) {
            ...
            lathe_update_status_t s;
            lathe_update_snapshot(&s);
            if (!s.connected)
                esp_wifi_connect();
        }
        ...
        if (read_fd >= 0) {
            ...
            lathe_diagnostics_serve(read_fd);
...
void lathe_network_start(void)
{
    ...
    if (validation_pending)
        lathe_operation_claim(LATHE_OWNER_UPDATE);
    if (validation_pending)
        reject_validation = lathe_storage_consume_ota_rejection();
    if (lathe_storage_wifi(wifi_ssid, wifi_password))
        atomic_store(&reconfigure, true);
    message("Configure Wi-Fi through USB");
    xTaskCreatePinnedToCore(network_task, "LATHE_network", 12288, NULL, 2, NULL, 0);
}
void lathe_network_poll(void)
{
    bool idle = state_get() == STATE_IDLE && p4_motion_idle() && !st_is_stepping() &&
                !plan_get_current_block() && !lathe_cycle_busy() && lathe_bridge_empty() && !lathe_serial_pending();
    lathe_diagnostics_poll(idle && !lathe_update_active());
    ...
    if (validation_pending && !reject_validation && idle && lathe_ui_ready() && lathe_storage_ready() &&
        !sys.alarm) {
        if (!validation_at)
            validation_at = now;
    ...
    if (atomic_load(&requested) && idle && !atomic_load(&enabled) && !atomic_load(&uploading)) {
        ...
        bool activate = atomic_load(&requested) && !atomic_load(&enabled) &&
                        !atomic_load(&uploading) && lathe_operation_claim(LATHE_OWNER_UPDATE);
        if (activate) {
...
status_code_t lathe_network_command(sys_state_t state, char *line)
{
    if (!strcmp(line, "P4DIAG=1")) { lathe_diagnostics_start(); return Status_OK; }
    if (!strcmp(line, "P4DIAG=0")) { lathe_diagnostics_stop(); return Status_OK; }
    if (!strcmp(line, "P4OTATEST=REJECT"))
        return lathe_storage_reject_next_ota(true) ? Status_OK : Status_IdleError;
    if (!strcmp(line, "P4OTATEST=CLEAR"))
        return lathe_storage_reject_next_ota(false) ? Status_OK : Status_IdleError;
    if (!strcmp(line, "P4OTA=1")) {
        return lathe_update_request() ? Status_OK : Status_IdleError;
    }
    ...
    if (!strncmp(line, "P4WIFI=", 7)) {
        if (state != STATE_IDLE || lathe_cycle_busy() || atomic_load(&uploading))
        ...
        *password++ = 0;
        ...
        if (!a || a > 32 || b > 63 || !unhex((uint8_t *)ssid, line + 7, a) ||
            ...
            return Status_InvalidStatement;
        if (!lathe_storage_set_wifi(ssid, secret))
