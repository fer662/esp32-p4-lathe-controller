...
status_code_t lathe_tmc_command(sys_state_t state, char *line)
{
    if (strcmp(line, "P4TMC"))
        return Status_Unhandled;
    if (state != STATE_IDLE || st_is_stepping() || lathe_cycle_busy())
    ...
    snprintf(report, sizeof(report),
             ...
             (unsigned long)driver.drv_status.reg.value, LATHE_BENCH_ONLY?"LOCKED":"CONTROLLED");
