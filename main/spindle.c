...
bool lathe_spindle_entry_cutting(void) { return entry_cutting; }
void lathe_spindle_entry_prepare(void) { entry_armed = entry_cutting = false; }
...
void lathe_spindle_profile(bool enabled) {
    ...
    if (lathe_spindle_rpm() != 0) profile_last_rpm = fabsf(lathe_spindle_rpm());
}
static volatile float follow_last_rpm=30;
void lathe_spindle_follow(bool enabled) {follow_mode=enabled;follow_braking=false;follow_last_rpm=30;}
void lathe_spindle_follow_braking(void) {follow_braking=true;}
...
float lathe_spindle_rpm(void) { return measured_rpm; }
float IRAM_ATTR lathe_spindle_profile_rpm(void)
{
...
int64_t lathe_spindle_position(void) { return position(); }
...
bool lathe_spindle_waiting_index(void) { return waiting; }
bool lathe_spindle_near_index(void)
{
...
static spindle_data_t *IRAM_ATTR get_data(spindle_data_request_t request)
{
    ...
    if (request == SpindleData_AngularPosition)
        ...
        float rpm=fabsf(profile_mode ? lathe_spindle_profile_rpm() : measured_rpm);
...
static void reset_data(void)
{
    ...
    if (entry_armed) {
        double rpm = fabsf(lathe_spindle_profile_rpm());
        double velocity = entry_lead * rpm / 60.0;
        phase_compensation = velocity * velocity / (2 * entry_acceleration);
        int64_t cut_advance = llround(phase_compensation * P4_ENCODER_CPR / entry_lead);
        entry_advance = llround(entry_seconds * rpm * P4_ENCODER_CPR / 60.0);
        int64_t shift = (cut_advance + entry_advance) % P4_ENCODER_CPR;
        index_phase = (phase + P4_ENCODER_CPR - shift) % P4_ENCODER_CPR;
        ...
        int32_t advance = (int32_t)lroundf(phase_compensation * P4_ENCODER_CPR / block->programmed_rate);
        index_phase = (phase + P4_ENCODER_CPR - (advance % P4_ENCODER_CPR)) % P4_ENCODER_CPR;
...
void lathe_spindle_init(void)
{
    ...
    spindle_register(&spindle, "External lathe spindle (encoder only)");
}
...
void IRAM_ATTR lathe_spindle_idle(void) { tracking = waiting = false; }
void IRAM_ATTR lathe_spindle_block(stepper_t *stepper)
{
    ...
    if (entry_armed && stepper->exec_block->sync_preload) {
        ...
        int64_t counts = oriented_position();
        origin = floor_turn(counts - index_phase) * P4_ENCODER_CPR + index_phase + entry_advance;
        entry_armed = false;
    }
...
void IRAM_ATTR lathe_spindle_edge(axes_signals_t steps)
{
    if (!tracking) return;
...
// It never commands STEP/DIR and is only available behind LATHE_BENCH_ONLY.
#if LATHE_BENCH_ONLY
...
bool lathe_spindle_simulator_active(void) { return simulator != NULL; }
static bool IRAM_ATTR sim_alarm(gptimer_handle_t timer, const gptimer_alarm_event_data_t *event, void *ctx)
{
    ...
    gpio_ll_set_level(&GPIO, P4_ENCODER_A, !!(bits & 2));
    gpio_ll_set_level(&GPIO, P4_ENCODER_B, !!(bits & 1));
...
static bool simulate(float rpm)
{
    ...
    if (!simulator) {
        ...
        gpio_set_level(P4_ENCODER_A, 0); gpio_set_level(P4_ENCODER_B, 0);
    ...
    if (rpm) {
        ...
        uint32_t interval = (uint32_t)lroundf(300000000.0f / (P4_ENCODER_CPR * fabsf(rpm)));
...
bool lathe_spindle_simulator_active(void) { return false; }
#endif
void lathe_spindle_poll(void)
{
    ...
    if (now - last_poll >= 2) {
        ...
        if (delta) {
            ...
            if (edge_interval_ms)
                last_edge_rpm = (delta > 0 ? 1 : -1) * 60000.0f / (P4_ENCODER_CPR * edge_interval_ms);
            last_change = now;
        }
        if (now - rpm_time >= 50) {
        ...
        if (tracking && !lathe_follow_busy() && !profile_mode) {
            if (oriented > furthest) furthest = oriented;
            if (now - last_change > 100) { sync_fault = "STALL"; p4_motion_fault(); }
            else if (furthest - oriented > 3) { sync_fault = "REVERSED"; p4_motion_fault(); }
        }
    }
    #if LATHE_BENCH_ONLY
...
status_code_t lathe_spindle_command(sys_state_t state, char *line)
{
    ...
    if (!strncmp(line, "P4PHASE=", 8)) {
        ...
        if (*end || end == line + 8 || value < 0 || value >= P4_ENCODER_CPR) return Status_InvalidStatement;
        phase = value;
        return Status_OK;
    }
...
void lathe_spindle_snapshot(lathe_diagnostics_t *s)
{
    ...
    s->tracking = tracking; s->waiting = waiting; s->simulated = lathe_spindle_simulator_active();
