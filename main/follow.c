...
static lathe_follow_config_t request, config;
...
static void say(const char *s, bool active)
{
    ...
    lathe_critical_enter(&lock, 3000 + __LINE__);
    published.active = active;
    memcpy(published.message, next, sizeof(next));
    lathe_critical_exit(&lock);
    if (!active) {
        lathe_operation_release(LATHE_OWNER_FOLLOW);
        lathe_spindle_follow(false);
    }
    char line[150];
    snprintf(line, sizeof(line), "[P4FOLLOW:ACTIVE:%u|STAGE:%s]\r\n", active, s);
...
void lathe_follow_snapshot(lathe_cycle_status_t *s)
{
    lathe_critical_enter(&lock, 3000 + __LINE__);
    *s = published;
    lathe_critical_exit(&lock);
}
bool lathe_follow_request(const lathe_follow_config_t *c)
{
    if (lathe_cycle_busy() || lathe_update_active() || lathe_axis_change_pending())
        return false;
    if (!lathe_operation_claim(LATHE_OWNER_FOLLOW))
        return false;
    lathe_critical_enter(&lock, 3000 + __LINE__);
    request = *c;
...
{
    if (!lathe_follow_busy() || (axis != 'X' && axis != 'Z') || !isfinite(distance) || distance <= 0)
        return false;
    lathe_critical_enter(&lock, 3000 + __LINE__);
    jog_axis = axis;
    jog_sign = sign > 0 ? 1 : -1;
...
void lathe_follow_reset(void)
{
    ...
    if (internal_reset && !stop_requested && !sys.alarm) {
        ...
        lathe_spindle_follow(config.mode != 2);
...
static bool register_position(int64_t counts, int64_t previous)
{
    if (position_registered)
        return true;
    double physical = anchor_counts + (pos(2) - anchor_z) / config.pitch * P4_ENCODER_CPR;
    double turns = ((double)counts - physical) / P4_ENCODER_CPR;
    ...
    if (fabs(error_mm) > tolerance) {
        double before = ((double)previous - physical) / P4_ENCODER_CPR;
        if (floor(before) == floor(turns))
            return false;
        whole = counts > previous ? floor(turns) : ceil(turns);
    }
    anchor_counts += whole * P4_ENCODER_CPR;
...
static void follow_position(int64_t counts, int64_t previous, double maximum)
{
    ...
    double z = anchor_z + ((double)counts - anchor_counts) / P4_ENCODER_CPR * config.pitch;
    double bounded = fmin(hi, fmax(lo, z));
    // The original controller discounts complete turns beyond a stop, retaining the partial
    ...
    if (idle() && fabs(pos(2) - bounded) <= .5 / settings.axis[2].steps_per_mm) {
        ...
        anchor_counts += turns * P4_ENCODER_CPR;
    }
...
{
    if (!lathe_follow_busy() || sys.abort)
        return;
    lathe_critical_enter(&lock, 3000 + __LINE__);
    ...
    lathe_follow_config_t c = request;
    if (begin)
        pending = false;
    lathe_critical_exit(&lock);
    if (sys.alarm) {
    ...
    if (begin || update) {
        if (!idle() || !lathe_bridge_empty() || lathe_serial_pending() || gc_state.modal.scaling_active) {
        ...
        anchor_x = pos(0);
        anchor_z = pos(2);
        anchor_counts = lathe_spindle_position();
        previous_counts = (int64_t)anchor_counts;
        char reference[180];
        snprintf(reference, sizeof(reference), "[P4FOLLOWREF:Z:%.6f|COUNTS:%.0f|PITCH:%.6f]\r\n", anchor_z,
                 anchor_counts, config.pitch);
        hal.stream.write(reference);
        stage = 0;
        position_follow = config.mode != 2 && fabs(lathe_spindle_rpm()) < 30;
        position_registered = true;
        stopping = internal_reset = waiting_ack = manual_motion = false;
        say("Armed; waiting for spindle", true);
    }
    double rpm = lathe_spindle_rpm();
    int64_t counts = lathe_spindle_position();
    bool reversing =
    ...
    if (jog) {
        ...
        snprintf(line, sizeof(line), "$J=G21G91%c%.6fF%.3f", axis, sign * distance,
                 lathe_jog_feed(axis, settings.axis[a].max_rate, false));
        manual_motion = true;
        stage = 0;
        send(line);
