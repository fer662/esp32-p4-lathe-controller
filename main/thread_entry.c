...
static status_code_t queue_entry(double x, double z, double lead)
{
    ...
    float rpm = fabsf(lathe_spindle_profile_rpm());
    ...
    lathe_spindle_entry_arm(duration, lead, settings.axis[Z_AXIS].acceleration / 3600.0);
    memcpy(gc_state.position, target, sizeof(target));
    return Status_OK;
}

status_code_t lathe_thread_entry_execute(double x, double z, double lead)
{
