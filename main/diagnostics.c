...
static lathe_diagnostics_t published;
static atomic_uint expires;
static uint32_t now_ms(void) { return esp_timer_get_time() / 1000; }
void lathe_diagnostics_start(void) { atomic_store(&expires, now_ms() + 1800000); }
void lathe_diagnostics_stop(void) { atomic_store(&expires, 0); }
bool lathe_diagnostics_active(void)
{
...
void lathe_diagnostics_snapshot(lathe_diagnostics_t *s)
{
    lathe_critical_enter(&lock, 6000 + __LINE__);
    *s = published;
    lathe_critical_exit(&lock);
}
void lathe_diagnostics_poll(bool idle)
{
    static uint32_t last_sample, last_tmc;
    uint32_t now = now_ms();
    if (!lathe_diagnostics_active() || now - last_sample < 250) return;
    last_sample = now;
    lathe_diagnostics_t s = { .sampled_ms = now };
    lathe_driver_snapshot(&s);
    lathe_spindle_snapshot(&s);
    bool refresh = idle && now - last_tmc >= 2000;
    if (refresh) last_tmc = now;
    lathe_tmc_snapshot(&s, refresh);
    s.tmc_sampled_ms = last_tmc;
    lathe_critical_snapshot(s.critical);
    lathe_critical_enter(&lock, 6000 + __LINE__);
    published = s;
    lathe_critical_exit(&lock);
}
...
void lathe_diagnostics_serve(int fd)
{
    if (!lathe_diagnostics_active()) {
    ...
    lathe_diagnostics_t s; lathe_diagnostics_snapshot(&s);
    lathe_status_t state; lathe_bridge_snapshot(&state);
    lathe_update_status_t update; lathe_update_snapshot(&update);
    char body[2300];
    snprintf(body, sizeof(body),
        ...
        p4_motor_controls_enabled()?"false":"true", s.enable_x, s.enable_z, (long)state.position[0], (long)state.position[2],
        ...
        (unsigned)(s.critical[0] >> 16), (unsigned)(s.critical[1] >> 16), (unsigned)(s.critical[0] & 65535), (unsigned)(s.critical[1] & 65535),
