...
{
    lathe_critical_enter(&lock, 5000 + __LINE__);
...
static bool valid_preferences(const lathe_preferences_t *p)
{
...
bool lathe_preferences_get(lathe_preferences_t *p)
{
    lathe_critical_enter(&lock, 5000 + __LINE__);
    bool ok = valid;
    if (ok)
...
void lathe_preferences_set(const lathe_preferences_t *p)
{
...
{
    return !lathe_update_active() &&
           (!sys.driver_started || (state_get() == STATE_IDLE && p4_motion_idle() && !st_is_stepping() &&
...
{
    if (!ready || !flash_idle() || !lathe_bridge_empty() || lathe_serial_pending())
        return;
...
status_code_t lathe_storage_command(sys_state_t state, char *line)
{
...
bool lathe_storage_consume_ota_rejection(void)
{
    uint8_t reject = 0;
