...
The old committed lathe controller implementation also fed X in before its spindle phase wait.
...
The earlier lathe controller baseline requested 50 mm/s² Z too, but initialized its step rate
...
P4PLAN diagnostics now report `APPROACH`, `FINISH`, `CUT_LENGTH` and `ACCEL`;
...
The original committed lathe controller cut used `posFromSpindle(..., true)` to clamp to its
...
The committed lathe controller baseline's `main/modes/ModeTurn.cpp` supplies the pass order,
...
logic or uncommitted lathe controller motion experiment is used.
...
```sh
cc -std=c11 -Wall -Wextra -Werror -Icomponents/lathe_ui \
  components/lathe_ui/cycle_plan.c tests/cycle_plan_test.c -lm -o /tmp/lathe-cycle-plan-test
/tmp/lathe-cycle-plan-test
