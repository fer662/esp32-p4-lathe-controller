...
def validate_cycle(lines, before, passes, starts, pitch, rpm, forward, length, threading):
    plan = fields(next(line for line in lines if line.startswith('[P4PLAN:')))
    lead = float(plan['P4PLAN'].split(':', 1)[1])
    ...
    done = [fields(line) for line in lines if line.startswith('[P4DONE:')]
    ...
    for i, cut in enumerate(cuts):
        assert int(cut['P4DONE'].split(':')[-1]) == i//starts+1
...
try:
    ...
    assert any('LATHE_P4_BENCH_MOTOR_ENABLES_LOCKED' in line for line in command('$I'))
