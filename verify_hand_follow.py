...
def arm(mode, pitch, rpm, bounds=(-10, 10)):
    ...
    ref = next(s for s in b.history if s.startswith('[P4FOLLOWREF:'))
    fields = dict(v.split(':', 1) for v in ref[1:-1].split('|'))
    return float(fields['P4FOLLOWREF'].split(':')[1]), int(fields['COUNTS'])
...
try:
    ...
    time.sleep(1)
    b.command('$P4SIM=60')
    b.receive(lambda s: s == '[P4FOLLOW:ACTIVE:1|STAGE:Following]', 8)
    time.sleep(3)
    b.command('$P4SIM=15')
    ...
    for i in range(12):
        time.sleep(.15 + i * .013)
        b.command('$P4SIM=60')
        b.receive(lambda s: s == '[P4FOLLOW:ACTIVE:1|STAGE:Following]', 5)
        time.sleep(.11 + i * .007)
        b.command('$P4SIM=15')
        b.receive(lambda s: 'STAGE:Following spindle position' in s, 5)
    stop()
