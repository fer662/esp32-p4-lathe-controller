...
static lathe_cycle_plan_t profile;
...
float lathe_spindle_profile_rpm(void) {return rpm;}
ENTRY_ARM
ENTRY_RESET
ENTRY_BLOCK
...
static void run(double speed,int direction,unsigned pass,double clearance,unsigned phase_counts) {
 lathe_cycle_config_t c={.operation=LATHE_THREAD,.passes=5,.starts=thread_starts,.pitch=.5*direction,.aux_forward=aux_forward,
  .x_min=0,.x_max=1,.z_min=0,.z_max=thread_starts>1?40:10};
 lathe_cycle_machine_t m={0,0,speed,100,960,200,settings.axis[X_AXIS].acceleration/3600,settings.axis[X_AXIS].max_rate,1200};char error[96];
 assert(lathe_cycle_plan(&c,&m,&profile,error,sizeof error));
 ...
 st_reset();
 double x=lathe_cycle_depth(&profile,pass), z=profile.approach;
 ...
 if(speed==50 && pass==0 && direction==1) {
  assert(lathe_thread_entry_execute(x,profile.finish,-1)==Status_GcodeMaxFeedRateExceeded);
  assert(!plan_get_current_block());
  reject_target=true;
  assert(lathe_thread_entry_execute(x,profile.finish,profile.lead)==Status_IdleError);
  assert(!plan_get_current_block());reject_target=false;sys.abort=false;
 }
 assert(lathe_thread_entry_execute(x,profile.finish,profile.lead)==Status_OK);
 bool done=!sys.abort;
 if(cancel_after!=0) {
