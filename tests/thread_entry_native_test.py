...
with tempfile.TemporaryDirectory() as directory:
    ...
    core=(root/'dependencies/grbl/state_machine.c').read_text()
    text=(p4/'tests/thread_entry_native_test.c').read_text()
    text=text.replace('ENTRY_ARM',spindle[spindle.index('void lathe_spindle_entry_arm('):spindle.index('static volatile float profile_last_rpm')])
    text=text.replace('ENTRY_RESET',spindle[spindle.index('static void reset_data('):spindle.index('static void set_state(')])
    text=text.replace('ENTRY_BLOCK',spindle[spindle.index('void IRAM_ATTR lathe_spindle_block('):spindle.index('void IRAM_ATTR lathe_spindle_edge(')])
    ...
    sources=[harness,p4/'main/thread_entry.c',p4/'components/lathe_ui/cycle_plan.c']
