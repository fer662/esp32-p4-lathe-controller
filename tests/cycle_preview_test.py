...
static double lathe_ui_work_offset(Axis *a) { return work_offset[a==&x ? 0 : 2]; }
static const char *lathe_ui_work_system() { return "G54"; }
static constexpr int MEASURE_METRIC=0;
static int measure=MEASURE_METRIC;
FORMATTER
int main() {
    lathe_cycle_config_t c={};
    c.operation=LATHE_TURN;c.passes=10;c.starts=1;c.pitch=.1;c.aux_forward=true;
    c.x_min=0;c.x_max=1;c.z_min=-1.5;c.z_max=8.5;c.rpm_limit=500;
    lathe_cycle_machine_t m={0,0,400,50,960,200,25,60,1200};
    lathe_cycle_plan_t plan;char error[96],text[1100];
    ...
    c.pitch=-.5;assert(lathe_cycle_plan(&c,&m,&plan,error,sizeof error));
    format_cycle_preview(plan,text,sizeof text);
...
with tempfile.TemporaryDirectory() as directory:
    ...
    include = str(root / 'components/lathe_ui')
    subprocess.run(['cc', '-std=c11', '-Wall', '-Wextra', '-Werror', '-I', include,
                    '-c', str(root / 'components/lathe_ui/cycle_plan.c'), '-o', str(path / 'plan.o')], check=True)
