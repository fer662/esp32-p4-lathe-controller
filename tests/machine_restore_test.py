...
init = source[source.index('void lathe_storage_init('):source.index('bool lathe_preferences_get(')]
poll = source[source.index('void lathe_storage_poll('):source.index('status_code_t lathe_storage_command(')]
harness = r'''
#include <assert.h>
#include <stdbool.h>
#include <stdint.h>
...
static lathe_preferences_t preferences;
static struct {bool driver_started;int32_t position[3];} sys;
static uint32_t xTaskGetTickCount(void) {return now;}
static void lathe_critical_enter(int *l,int tag) {(void)l;(void)tag;}
static void lathe_critical_exit(int *l) {(void)l;}
static bool flash_idle(void) {return !moving;}
static bool lathe_bridge_empty(void) {return !queued;}
static bool lathe_serial_pending(void) {return serial_pending;}
static bool valid_preferences(const lathe_preferences_t *p) {return p->version==1;}
STATE
static machine_state_t disk;
static size_t disk_size;
static bool disk_present,mutate_during_write;
static int nvs_flash_init_partition(const char *p) {assert(!strcmp(p,LATHE_SETTINGS_PARTITION));return 0;}
static int nvs_open_from_partition(const char *p,const char *n,int mode,int *h) {assert(!strcmp(p,LATHE_SETTINGS_PARTITION));assert(!strcmp(n,"controller"));(void)mode;*h=1;return 0;}
...
static int nvs_set_blob(int h,const char *key,const void *p,size_t size) {
 ...
 if(!strcmp(key,"machine_v1")) {
  ...
  if(mutate_during_write) {mutate_during_write=false;lathe_saved_disabled_set(5);}
 }
 return 0;
}
...
static void boot(void) {
 ...
 lathe_storage_init();lathe_storage_restore_position(&sys.position);sys.driver_started=true;
}
int main(void) {
 boot();assert(!machine_valid && !lathe_saved_disabled_get());
 int32_t limits[4];lathe_saved_limits_get(limits);assert(limits[0]==INT32_MIN && limits[3]==INT32_MAX);
 sys.position[0]=-12345;sys.position[2]=54321;
 const int32_t bounds[]={-12000,24000,INT32_MIN,100000};
 lathe_saved_limits_set(bounds);lathe_saved_disabled_set(1);
 lathe_storage_poll();assert(!disk_present);now=499;lathe_storage_poll();assert(!disk_present);
 now=500;lathe_storage_poll();assert(disk_present && writes==1 && !machine_dirty);
 for(unsigned i=0;i<10;i++) {now+=1000;lathe_storage_poll();}assert(writes==1);
 boot();assert(sys.position[0]==-12345 && sys.position[2]==54321);
 assert(lathe_saved_disabled_get()==1);lathe_saved_limits_get(limits);assert(!memcmp(limits,bounds,sizeof bounds));
 // Motion cannot save partial positions, nor can queued commands race a flash write.
 moving=true;sys.position[2]=999;now=1000;lathe_storage_poll();assert(writes==1);
 moving=false;queued=true;lathe_storage_poll();assert(writes==1);
 queued=false;lathe_storage_poll();now+=500;fail_write=true;lathe_storage_poll();
 assert(machine_dirty && failures==1 && writes==1);
 fail_write=false;lathe_storage_poll();assert(!machine_dirty && disk.position[2]==999 && writes==2);
 // A UI update concurrent with flash commit remains dirty and is saved next.
 lathe_saved_disabled_set(4);now+=500;mutate_during_write=true;lathe_storage_poll();
 assert(machine_dirty && disk.disabled==4);now+=500;lathe_storage_poll();assert(!machine_dirty && disk.disabled==5);
 boot();assert(lathe_saved_disabled_get()==5 && sys.position[2]==999);
 // Crossed limits are a valid editable UI state: preserve them without losing position.
 const int32_t crossed[]={24000,-12000,0,0};lathe_saved_limits_set(crossed);now+=500;lathe_storage_poll();
 boot();lathe_saved_limits_get(limits);assert(!memcmp(limits,crossed,sizeof crossed) && sys.position[2]==999);
