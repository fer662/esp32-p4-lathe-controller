...
validator=source[source.index('static bool valid_preferences('):source.index('bool lathe_storage_ready(')]
upgrade=source[source.index('void lathe_storage_upgrade_motion('):source.index('void lathe_storage_poll(')]
harness=r'''
#include <assert.h>
#include <stdbool.h>
#include <stdint.h>
...
int main(void) {
 lathe_preferences_t prefs={.version=1,.mode=0,.measure=0,.pitch_type=0,.pitch=1000,
 ...
 reset();lathe_storage_upgrade_motion();
 if(LATHE_BENCH_ONLY) {
 ...
 lathe_storage_upgrade_motion();assert(setter_calls==2);
 ...
 reset();stored_revision=2;read_result=ESP_OK;lathe_storage_upgrade_motion();
 ...
 for(unsigned custom=0;custom<3;custom++) {
  ...
  lathe_storage_upgrade_motion();assert(!setter_calls && !sync_calls && stored_revision==4);
 ...
 reset();ready=false;lathe_storage_upgrade_motion();assert(!setter_calls && !marker_calls);
 reset();read_result=ESP_FAIL;lathe_storage_upgrade_motion();assert(!setter_calls && !marker_calls);
 reset();stored_revision=4;read_result=ESP_OK;lathe_storage_upgrade_motion();assert(!setter_calls && !marker_calls);
 for(unsigned revision=0;revision<=3;revision++) {
  reset();stored_revision=revision;read_result=ESP_OK;setter_ok=false;settings.axis[0].acceleration=500*3600;
  lathe_storage_upgrade_motion();assert(setter_calls==1 && !sync_calls && !marker_calls);
 }
 reset();stored_revision=3;read_result=ESP_OK;settings.axis[0].acceleration=500*3600;
 core_write_ok=false;lathe_storage_upgrade_motion();assert(sync_calls==1 && !writes && !marker_calls);
 // Next boot retries old persisted values when a core write failed.
 settings.axis[0].acceleration=500*3600;
 core_write_ok=true;lathe_storage_upgrade_motion();
 assert(setter_calls==2 && writes==1 && stored_revision==4);
 reset();stored_revision=3;read_result=ESP_OK;settings.axis[0].acceleration=500*3600;
 marker_write_ok=false;lathe_storage_upgrade_motion();assert(writes==1 && failures==1 && stored_revision==3);
 marker_write_ok=true;lathe_storage_upgrade_motion();assert(setter_calls==1 && stored_revision==4);
...
with tempfile.TemporaryDirectory() as directory:
    ...
    for bench in (0,1):
        subprocess.run(['cc','-std=c11','-Wall','-Wextra','-Werror','-I',str(root/'components/lathe_ui'),f'-DLATHE_BENCH_ONLY={bench}',str(path/'test.c'),'-o',str(path/'test')],check=True)
