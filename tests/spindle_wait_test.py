...
core = (root / 'dependencies/grbl/state_machine.c').resolve().read_text()
...
a = spindle.index('float IRAM_ATTR lathe_spindle_profile_rpm(')
...
int main(void) {
 ...
 assert(lathe_spindle_profile_rpm()==0);
 edge_interval_ms=100;last_edge_rpm=.5;now=1150;assert(lathe_spindle_profile_rpm()==.5);
 last_edge_rpm=-.5;assert(lathe_spindle_profile_rpm()==-.5);
 now=1300;assert(lathe_spindle_profile_rpm()==0);
 measured_rpm=400;assert(lathe_spindle_profile_rpm()==400);
 measured_rpm=0;last_edge_rpm=400;now=1001;assert(lathe_spindle_profile_rpm()==0);
