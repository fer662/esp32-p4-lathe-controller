...
const char *lathe_ui_work_system() {return "G54";}
double lathe_ui_work_offset(Axis *a) {return work_offset[a==&x?0:2];}
...
extern "C" void lathe_audio_tone(unsigned,unsigned) {}
extern "C" bool lathe_cycle_busy() {return false;}
extern "C" bool lathe_cycle_advance() {return true;}
extern "C" bool lathe_axis_change_pending() {return false;}
void lathe_ui_sync() {
...
void lathe_ui_jog(Axis *a,int sign,bool pressed) {jogs.push_back({a->name,sign,pressed});}
...
bool lathe_ui_limits_editable() {return limitsEditable;}
bool lathe_ui_set_jog_limits(bool enabled) {if(!limitsEditable)return false;jogLimitsEnabled=enabled;return true;}
double lathe_ui_limit_coordinate(Axis *a,long value) {double mm=(double)value/steps(a)-lathe_ui_work_offset(a);return measure==MEASURE_METRIC?mm:mm/25.4;}
bool lathe_ui_limit_steps(Axis *a,double value,long *out) {
    double raw=(value*(measure==MEASURE_METRIC?1:25.4)+lathe_ui_work_offset(a))*steps(a);
    if(!std::isfinite(raw)||raw<=INT32_MIN||raw>=INT32_MAX)return false;
    *out=lround(raw);return true;
}
const char *lathe_ui_apply_limits(const long v[4]) {
