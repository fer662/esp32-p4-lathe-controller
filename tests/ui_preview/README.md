...
Builds the production LVGL widgets with the pinned LVGL version and a simulated
...
Run `idf.py reconfigure` once to resolve `managed_components/lvgl__lvgl`, then:

```sh
cmake -S tests/ui_preview -B /tmp/lathe-ui-preview-build
cmake --build /tmp/lathe-ui-preview-build --target ui_preview -j 8
cd /tmp/lathe-ui-preview-build
./ui_preview
```
...
Run `python3 tests/jog_modes_test.py` for the production jog-routing
