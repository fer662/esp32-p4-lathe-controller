...
static bool lathe_cycle_busy(void) { return busy; }
static void lathe_spindle_entry_prepare(void) {}
static float lathe_spindle_profile_rpm(void) { return rpm; }
static uint32_t lathe_bridge_cycle_submit(const char *line) {
...
static void lathe_cycle_cancel(void) { assert(!"Unexpected submission failure"); }
EMIT
...
static void move(unsigned next) {
    ...
    if(!strcmp(emitted,"$P4THREADENTRY")) {
        ...
        position[0]=lathe_cycle_depth(&plan,pass); position[1]=plan.finish;
        ...
        assert(fabs(position[0]-lathe_cycle_depth(&plan,pass))<1e-6);
...
int main(void) {
    lathe_cycle_machine_t m={.x=1,.z=10,.rpm=400,.z_acceleration=50,.z_max_rate=960,.z_steps_mm=200,
    ...
    for(unsigned op=LATHE_TURN;op<=LATHE_ELLIPSE;op++) {
        ...
        for(unsigned aux=0;aux<2;aux++) {
            lathe_cycle_config_t c={.operation=op,.passes=3,.starts=1,.pitch=.1*sign,.aux_forward=aux,
                .x_min=0,.x_max=1,.z_min=0,.z_max=10,.rpm_limit=500};
            rpm=m.rpm=400*spindle;
            if(op==LATHE_CUT) c.z_min=c.z_max=m.z;
            assert(lathe_cycle_plan(&c,&m,&plan,error,sizeof error));
            ...
            for(pass=0;pass<c.passes;pass++) {
                if(pass && op==LATHE_ELLIPSE) { move(2); move(3); }
                ...
                issue(6);
                for(segment=0;segment<(op==LATHE_ELLIPSE?plan.segments:1);segment++) move(7);
                double expected=op==LATHE_CUT ? plan.cut_start+(plan.cut_end-plan.cut_start)*(pass+1)/c.passes : plan.cut_end;
            ...
            if(op==LATHE_CUT) assert(fabs(position[1]-m.z)<1e-6);
        }
    }
    // Multi-start Thread takes up inside the span and retains G33 lead/registration.
    lathe_cycle_config_t c={.threading=true,.passes=4,.starts=2,.pitch=.5,.aux_forward=true,
        .x_min=-1,.x_max=1,.z_min=10,.z_max=20,.rpm_limit=360};
    rpm=m.rpm=300; assert(lathe_cycle_plan(&c,&m,&plan,error,sizeof error));
...
with tempfile.TemporaryDirectory() as directory:
    ...
    subprocess.run(['cc', '-std=c11', '-Wall', '-Wextra', '-Werror',
                    '-I', str(root / 'components/lathe_ui'), str(path / 'test.c'),
                    str(root / 'components/lathe_ui/cycle_plan.c'), '-lm', '-o', str(path / 'test')], check=True)
