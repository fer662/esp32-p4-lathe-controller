...
source = (root / 'main/controller.c').read_text()
platform = (root / 'dependencies/esp32/p4/components/grblhal_p4/driver.c').read_text()
start = platform.index('static void IRAM_ATTR enable(')
callback = platform[start:platform.index('static void IRAM_ATTR reset_direction', start)]
start = source.index('bool lathe_axis_change_pending(')
callback += source[start:source.index('static bool motion_allowed(', start)]
...
#define P4_BENCH_ONLY LATHE_BENCH_ONLY
...
static uint8_t disabled_requested, disabled_applied, disabled_axes;
...
static bool p4_motion_idle(void) { return !timer_active; }
static bool st_is_stepping(void) { return stepping; }
static bool plan_get_current_block(void) { return planner_active; }
static bool lathe_cycle_busy(void) { return cycle_active; }
static unsigned state_get(void) { return state; }
static void lathe_cycle_cancel(void) { cancels++; }
static void lathe_spindle_follow_braking(void) { brakes++; }
...
static void lathe_saved_disabled_set(uint8_t mask) { saved_disabled=mask; }
...
static bool lathe_ui_ready(void) { return true; }
...
int main(void)
{
                ...
                for (unsigned hold=0; hold<2; hold++) {
                    for (unsigned requested=0; requested<8; requested++) {
                      for (unsigned disabled=0; disabled<8; disabled++) {
                        disabled_applied=disabled_axes=disabled;
                        writes=0;
                        enable((axes_signals_t){.mask=requested},hold);
                        bool blocked=LATHE_BENCH_ONLY || failed || !ready;
    ...
    disabled_applied=disabled_axes=0;fault=false;state=STATE_CYCLE;
    timer_active=stepping=planner_active=cycle_active=true;
    enable((axes_signals_t){.mask=5},false);
    lathe_axis_set_disabled('X',true);
    assert(saved_disabled & X_AXIS_BIT);
    assert(lathe_axis_change_pending() && !disabled_applied);
    axis_enable_poll();
    ...
    axis_enable_poll();
    assert(disabled_applied==1 && !lathe_axis_change_pending());
    assert(pins[47]==1 && pins[30]==!LATHE_BENCH_ONLY);
    ...
    lathe_axis_set_disabled('X',false);
    lathe_axis_set_disabled('X',true);
    lathe_axis_set_disabled('X',false);
    axis_enable_poll();
    ...
    assert(!disabled_applied && !lathe_axis_change_pending());
    assert(pins[47]==LATHE_BENCH_ONLY && pins[30]==!LATHE_BENCH_ONLY);
    // Simultaneous X/Z requests and a newer request during cancellation.
    lathe_axis_set_disabled('X',true);lathe_axis_set_disabled('Z',true);
    axis_enable_poll();sys.rt_exec_state=0;
    lathe_axis_set_disabled('X',false);
    axis_enable_poll();
    assert(lathe_axis_change_pending() && !disabled_applied);
    sys.rt_exec_state=0;axis_enable_poll();
    assert(disabled_applied==4 && !lathe_axis_change_pending());
    assert(pins[47]==LATHE_BENCH_ONLY && pins[30]==0);
    lathe_axis_set_disabled('Z',true);lathe_axis_set_disabled('Y',true);
    assert(!lathe_axis_change_pending()); // redundant/invalid request
    ...
    assert(validate(&commands,0,&block,0)==Status_SettingDisabled);
    lathe_axis_set_disabled('Z',false);
...
with tempfile.TemporaryDirectory(prefix='lathe-enable-test-') as directory:
    ...
    for bench in (0,1):
        ...
        subprocess.run(['cc','-std=c11','-Wall','-Wextra','-Werror','-Wno-unused-parameter',
                        f'-DLATHE_BENCH_ONLY={bench}',str(code),'-o',str(executable)],check=True)
