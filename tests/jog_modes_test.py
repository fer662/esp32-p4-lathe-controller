...
source = (root / 'components/lathe_ui/ui.cpp').read_text()
...
functions += source[source.index('void buttonMoveStepPress('):source.index('void lathe_ui_sync(')]
harness = r'''
#include <cassert>
#include <cmath>
#include <climits>
...
static uint32_t lathe_bridge_submit(const char *s) {if(!accept)return 0;submitted.emplace_back(s);return ++next_id;}
static void lathe_bridge_snapshot(Status *s) {*s=published;}
static void lathe_bridge_cancel() {cancels++;}
static bool lathe_cycle_busy() {return follow;}
static bool lathe_follow_busy() {return follow;}
static bool lathe_axis_change_pending() {return axis_pending;}
static bool lathe_update_active() {return updating;}
static bool lathe_follow_jog(char a,int s,double d,bool held) {if(!accept)return false;manual.push_back({a,s,d,held});return true;}
static void lathe_follow_release() {releases++;}
FUNCTIONS
static void reset() {
...
int main() {
 ...
 assert(lathe_ui_limits_editable());assert(lathe_ui_limit_coordinate(&x,x.pos)==0);
 ...
 assert(lathe_ui_limits_editable());assert(!strcmp(lathe_ui_work_system(),"G55"));
 assert(lathe_ui_limit_coordinate(&z,500)==0);
 ...
 for(long step:{10000L,1000L,100L,25400L,2540L,254L}) {
  reset();moveStep=step;lathe_ui_jog(&x,1,true);assert(distance()==100);
  lathe_ui_jog(&x,1,false);assert(cancels==1 && !held_axis);
 }
 // Single means a complete selected increment, without repeat or release cancellation.
 for(long step:{10000L,1000L,100L,25400L,2540L,254L}) {
  reset();jogContinuous=false;moveStep=step;lathe_ui_jog(&z,-1,true);
  assert(fabs(distance()+step/10000.0)<1e-6);
  for(int i=0;i<20;i++)lathe_ui_jog(&z,-1,true);
  assert(submitted.size()==1);lathe_ui_jog(&z,-1,false);assert(!cancels && !held_axis);
 }
 ...
 for(bool single:{false,true}) for(Axis *axis:{&x,&z}) for(int sign:{-1,1}) {
  ...
  lathe_ui_jog(axis,sign,true);assert(continuous_jog && !single_jog_id);
  assert(submitted.back().find("F234.000")!=std::string::npos);
  assert(fabs(distance())==(axis==&x?1:2));
  moveStep=MOVE_STEP_1;lathe_ui_jog(axis,sign,false);assert(cancels==1);
 }
 reset();moveStep=MOVE_STEP_RAPIDS;jogLimitsEnabled=false;x.leftStop=0;
 lathe_ui_jog(&x,1,true);assert(distance()==100);lathe_ui_jog(&x,1,false);assert(cancels==1);
 reset();moveStep=MOVE_STEP_RAPIDS;follow=true;lathe_ui_jog(&x,1,true);
 ...
 for(float invalid:{0.0f,-1.0f,INFINITY,NAN}) {
  ...
  lathe_ui_jog(&x,1,true);assert(submitted.empty() && !held_axis);
 }
 // Ordinary X jog remains 60 mm/min with the new machine maximum.
 reset();lathe_ui_jog(&x,1,true);assert(submitted.back().find("F60.000")!=std::string::npos);
 // Fast taps cannot queue steps based on stale position, even if ACK precedes publication.
 reset();jogContinuous=false;lathe_ui_jog(&x,1,true);lathe_ui_jog(&x,1,false);
 lathe_ui_jog(&x,1,true);assert(submitted.size()==1 && !held_axis);
 published.completed_id=1;lathe_ui_jog(&x,1,true);assert(submitted.size()==1);
 published.command_id=1;lathe_ui_jog(&x,1,true);assert(submitted.size()==1);
 published.sampled_completed_id=1;published.moving=true;lathe_ui_jog(&x,1,true);assert(submitted.size()==1);
 published.moving=false;x.pos=1200;x.leftStop=1800;lathe_ui_jog(&x,1,true);
 assert(submitted.size()==2 && distance()==.5);lathe_ui_jog(&x,1,false);
 // An explicit stop clears a queued single-step reservation, including before ACK.
 cancel_ui_motion();assert(single_jog_id==0);lathe_ui_jog(&x,-1,true);assert(submitted.size()==3);
 reset();x.disabled=true;lathe_ui_jog(&x,1,true);assert(submitted.empty());
 x.disabled=false;axis_pending=true;lathe_ui_jog(&x,1,true);assert(submitted.empty());
 axis_pending=false;published.alarm=1;lathe_ui_jog(&x,1,true);assert(submitted.empty());
 reset();published.ready=false;lathe_ui_jog(&x,1,true);assert(submitted.empty());
 reset();x.leftStop=0;lathe_ui_jog(&x,1,true);assert(submitted.empty());
 reset();accept=false;jogContinuous=false;lathe_ui_jog(&x,1,true);assert(!single_jog_id);
 // Changing mode affects the next press, not release of an already-held jog.
 reset();lathe_ui_jog(&x,1,true);jogContinuous=false;lathe_ui_jog(&x,1,false);assert(cancels==1);
 reset();jogContinuous=false;lathe_ui_jog(&x,1,true);jogContinuous=true;lathe_ui_jog(&x,1,false);assert(!cancels);
 // Assisted feed receives held=false for one-shot displacement, held=true for Hold.
 for(bool hold:{false,true}) {
  reset();follow=true;jogContinuous=hold;moveStep=1000;lathe_ui_jog(&z,1,true);
  assert(manual.size()==1 && manual[0].held==hold);
  assert(fabs(manual[0].distance-(hold?300:.1))<1e-6);
  lathe_ui_jog(&z,1,false);assert(releases==(hold?1u:0u) && !cancels);
 }
 reset();follow=true;accept=false;lathe_ui_jog(&z,1,true);lathe_ui_jog(&z,1,false);assert(!releases && !held_axis);
 ...
 static_assert(sizeof(Legacy)==sizeof(lathe_preferences_t));
 Legacy old{};old.version=1;old.pitch=15000;old.move_step=1000;old.reserved[0]=0;
 lathe_preferences_t p{};memcpy(&p,&old,sizeof(p));assert(p.jog_mode==0 && p.pitch==15000 && p.move_step==1000);
 ...
 assert(lathe_ui_set_jog_limits(false));assert(x.leftStop==600&&x.rightStop==-600);
 jogContinuous=false;lathe_ui_jog(&x,1,true);assert(distance()==1);lathe_ui_jog(&x,1,false);
 assert(!lathe_ui_set_jog_limits(true)); // Unacknowledged request.
 published.sampled_completed_id=1;published.moving=true;assert(!lathe_ui_set_jog_limits(true));
 published.moving=false;assert(lathe_ui_set_jog_limits(true));assert(distance_to_stop(&x,1,1)==.5);
 follow=true;assert(!lathe_ui_set_jog_limits(false));follow=false;
 updating=true;assert(!lathe_ui_set_jog_limits(false));updating=false;
 // Atomic apply, ordering, clearing, units, origin offsets and numeric range checks.
 reset();long bounds[]={-1200,2400,-6000,0};assert(!lathe_ui_apply_limits(bounds));
 assert(x.rightStop==-1200&&x.leftStop==2400&&z.rightStop==-6000&&z.leftStop==0&&submitted.empty());
 long invalid[]={3600,2400,-6000,200};assert(lathe_ui_apply_limits(invalid));assert(x.rightStop==-1200&&z.leftStop==0);
 published.moving=true;assert(lathe_ui_apply_limits(bounds));published.moving=false;
 bounds[0]=LONG_MIN;bounds[1]=LONG_MAX;assert(!lathe_ui_apply_limits(bounds));assert(x.rightStop==LONG_MIN&&x.leftStop==LONG_MAX);
 long raw=0;status.work_offset[0]=1;assert(lathe_ui_limit_steps(&x,-2,&raw)&&raw==-1200);assert(lathe_ui_limit_coordinate(&x,raw)==-2);
 measure=1;status.work_offset[2]=-1;assert(lathe_ui_limit_steps(&z,-1,&raw)&&raw==-5280);assert(fabs(lathe_ui_limit_coordinate(&z,raw)+1)<1e-9);
 assert(!lathe_ui_limit_steps(&x,INFINITY,&raw)&&!lathe_ui_limit_steps(&x,NAN,&raw)&&!lathe_ui_limit_steps(&x,1e20,&raw));
}
'''.replace('FUNCTIONS', functions)
with tempfile.TemporaryDirectory() as directory:
    temp = Path(directory)
    (temp / 'jog.cpp').write_text(harness)
    subprocess.run(['c++', '-std=c++17', '-Wall', '-Wextra', '-Werror', '-I', str(root / 'components/lathe_ui'), str(temp / 'jog.cpp'), '-o', str(temp / 'jog')], check=True)
    subprocess.run([str(temp / 'jog')], check=True)
follow = (root / 'main/follow.c').read_text()
follow_functions = follow[follow.index('bool lathe_follow_jog('):follow.index('void lathe_follow_reset(')]
follow_harness = r'''
#include <assert.h>
#include <stdbool.h>
#include <math.h>
...
static bool lathe_follow_busy(void) {return active;}
static void lathe_critical_enter(int *l,int site) {(void)l;(void)site;}
static void lathe_critical_exit(int *l) {(void)l;}
FUNCTIONS
int main(void) {
 assert(lathe_follow_jog('Z',1,300,true));
 ...
 lathe_follow_release();assert(!manual_pending && !manual_held && release_requested);
 assert(lathe_follow_jog('X',-1,.1,false));
 assert(manual_pending && !manual_held && !release_requested);
 assert(jog_axis=='X' && jog_sign==-1 && jog_distance==.1);
 active=false;assert(!lathe_follow_jog('X',1,.1,true));
}
