...
static lathe_cycle_config_t preview_config;
...
extern "C" void lathe_audio_start(void);
extern "C" bool lathe_audio_initialized(void);
static void send(const char *line)
{
    uint32_t id = lathe_bridge_submit(line);
    if (id) { last_command = id; notice.clear(); }
    else notice = lathe_cycle_busy() ? "Stop the cycle before jogging" : "Command queue is full";
}
...
bool lathe_ui_limits_editable()
{
    ...
    return status.ready && !status.alarm && !status.moving && !status.held &&
           !held_axis && !lathe_cycle_busy() && !lathe_axis_change_pending() &&
           !lathe_update_active() && status.sampled_completed_id >= last_command;
}
bool lathe_ui_set_jog_limits(bool enabled)
{
    if (!lathe_ui_limits_editable()) {
...
const char *lathe_ui_work_system()
{
...
double lathe_ui_work_offset(Axis *a) { return status.work_offset[a == &x ? 0 : 2]; }
double lathe_ui_limit_coordinate(Axis *a, long steps)
{
    double mm = (double)steps / steps_mm(a) - lathe_ui_work_offset(a);
    return measure == MEASURE_METRIC ? mm : mm / 25.4;
}
bool lathe_ui_limit_steps(Axis *a, double coordinate, long *steps)
{
    double raw = (coordinate * (measure == MEASURE_METRIC ? 1.0 : 25.4) + lathe_ui_work_offset(a)) * steps_mm(a);
...
const char *lathe_ui_apply_limits(const long limits[4])
{
    if (!lathe_ui_limits_editable()) return "Stop motion and assisted operations before applying limits.";
...
void lathe_ui_jog(Axis *a, int sign, bool pressed)
{
    if (!pressed) {
        ...
        if (continuous_jog) {
            if (lathe_follow_busy()) lathe_follow_release();
            else cancel_ui_motion();
        }
        return;
    }
    if (held_axis || a->disabled || lathe_axis_change_pending()) return;
    const bool rapid = moveStep == MOVE_STEP_RAPIDS;
    if (rapid && lathe_follow_busy()) { notice = "Stop assisted operation before rapid jogging"; return; }
    ...
    if (lathe_follow_busy()) {
        if (lathe_follow_jog(a->name, sign, distance, continuous_jog)) held_axis = a;
        return;
    }
    lathe_bridge_snapshot(&status);
    ...
    const float feed = lathe_jog_feed(a->name, status.max_rate[a == &x ? 0 : 2], rapid);
...
{
    if (!lathe_ui_limits_editable()) { notice = "Stop motion and wait for commands before setting G54 zero"; return; }
    char command[16];
...
void setAxisDisabled(Axis *a, bool disabled)
{
    ...
    single_jog_id = 0;
    held_axis = nullptr;
    lathe_axis_set_disabled(a->name, disabled);
    a->disabled = disabled;
...
String getAxisPos(Axis *a) { return format_decimal(lathe_ui_limit_coordinate(a, a->pos), 3); }
String getAxisLeftStop(Axis *a) { return a->leftStop == LONG_MAX ? "-" : format_decimal(lathe_ui_limit_coordinate(a, a->leftStop), 3); }
String getAxisRightStop(Axis *a) { return a->rightStop == LONG_MIN ? "-" : format_decimal(lathe_ui_limit_coordinate(a, a->rightStop), 3); }
...
{
    if (lathe_follow_busy()) {
        if (!lathe_follow_update(dupr / 10000.0, coneRatio, auxForward))
            notice = "Feed stopped: select a valid nonzero pitch";
    } else if (lathe_cycle_busy()) {
        cancel_ui_motion();
...
void setTurnPasses(int value) { if (turnPasses != value) { turnPasses = value; if (lathe_cycle_busy()) cancel_ui_motion(); } }
void setStarts(int value) { if (starts != value) { starts = value; if (lathe_cycle_busy()) cancel_ui_motion(); } }
...
static void format_cycle_preview(const lathe_cycle_plan_t &plan, char *text, size_t size)
{
    ...
    auto coordinate = [scale](char axis, double machine_mm) {
        Axis *a = axis == 'X' ? &x : &z;
        return (machine_mm - lathe_ui_work_offset(a)) * scale;
    };
    ...
    if (plan.indexed)
        snprintf(geometry,sizeof(geometry),
            ...
            digits,coordinate('X',lathe_cycle_depth(&plan,0)),unit,
            digits,coordinate('X',lathe_cycle_depth(&plan,plan.config.passes-1)),unit,
    ...
    snprintf(text,size,
        ...
        lathe_cycle_name(plan.config.operation),plan.config.passes,plan.starts,plan.lead*scale,unit,
        plan.cut_axis,digits,coordinate(plan.cut_axis,plan.cut_start),digits,coordinate(plan.cut_axis,plan.cut_end),unit,
        geometry,lathe_ui_work_system());
}
static void preview_cycle()
{
    ...
    preview_config.operation = mode==MODE_FACE ? LATHE_FACE : mode==MODE_CUT ? LATHE_CUT : mode==MODE_ELLIPSE ? LATHE_ELLIPSE : mode==MODE_THREAD ? LATHE_THREAD : LATHE_TURN;
    ...
    lathe_cycle_machine_t machine = {x.pos/steps_mm(&x),z.pos/steps_mm(&z),status.rpm,
        status.acceleration[2],status.max_rate[2],steps_mm(&z),status.acceleration[0],status.max_rate[0],steps_mm(&x)};
    lathe_cycle_plan_t plan;
    char error[96];
    if (!lathe_cycle_plan(&preview_config,&machine,&plan,error,sizeof(error))) { notice=error; return; }
    ...
    lv_obj_add_event_cb(run,[](lv_event_t *) {
        if (lathe_cycle_request(&preview_config)) { cycle_selected=true; isOn=true; notice.clear(); }
...
void buttonOnOffPress(bool on)
{
    ...
    if(lathe_follow_request(&config)) {cycle_selected=true;isOn=true;} else notice="Controller is busy";
}
...
void lathe_ui_sync()
{
    ...
    lathe_saved_limits_set(limits);
    lathe_preferences_t prefs={};prefs.version=1;prefs.mode=mode;prefs.measure=measure;prefs.pitch_type=pitchType;
    prefs.pitch=dupr;prefs.move_step=moveStep;prefs.passes=turnPasses;prefs.starts=starts;prefs.cone_ratio=coneRatio;
    prefs.aux_forward=auxForward;prefs.sound=buzzerEnabled;prefs.jog_mode=jogContinuous?0:1;lathe_preferences_set(&prefs);
    lathe_bridge_snapshot(&status);
    if (status.stream_generation != last_generation) {
        last_command = 0;
        single_jog_id = 0;
        if (!lathe_follow_manual_held()) held_axis = nullptr;
    ...
    lathe_cycle_status_t cycle; lathe_cycle_snapshot(&cycle);
    if (cycle.active) {
        cycle_selected=true;
        isOn=true;
        notice=cycle.message;
        if(!lathe_follow_busy()) notice+=" | Pass "+std::to_string(cycle.pass)+", start "+std::to_string(cycle.start);
    ...
    if(update_label) {
        lathe_update_status_t update;lathe_update_snapshot(&update);char text[400], pairing[80];
    ...
    if (diagnostics_label && !lv_obj_has_flag(diagnostics_panel, LV_OBJ_FLAG_HIDDEN)) {
        lathe_diagnostics_t d; lathe_diagnostics_snapshot(&d);
        lathe_update_status_t net; lathe_update_snapshot(&net);
        char text[1000];
        snprintf(text,sizeof(text),
            ...
            lathe_diagnostics_active()?"sharing":"sharing stopped", p4_motor_controls_enabled()?"Axis controls enabled":"Motor enables locked", net.connected?"connected":"disconnected",
    ...
    if (status_label) {
        ...
        text += p4_motor_controls_enabled() ? "  |  Axis controls enabled" : "  |  Motor outputs disabled";
        text += String("  |  ") + lathe_ui_work_system() + "  |  Tap for diagnostics";
        if (lathe_axis_change_pending()) text += "\nStopping before changing motor enable";
...
void lathe_ui_show_update()
{
    cancel_ui_motion();
    held_axis = nullptr;
    lathe_update_request();
    if (!update_panel) {
        ...
        lv_obj_add_event_cb(close, [](lv_event_t *) { lathe_update_cancel(); lv_obj_add_flag(update_panel, LV_OBJ_FLAG_HIDDEN); }, LV_EVENT_CLICKED, nullptr);
    }
...
static void show_diagnostics()
{
    ...
    if (!diagnostics_panel) {
        ...
        lv_obj_add_event_cb(diagnostics_stop,[](lv_event_t *) {
            lathe_diagnostics_stop();lv_obj_add_flag(diagnostics_panel,LV_OBJ_FLAG_HIDDEN);
        },LV_EVENT_CLICKED,nullptr);
    }
...
static void ui_task(void *)
{
    ...
    if (!bsp_display_start_with_config(&cfg)) { ESP_LOGE("LATHE_UI", "Display initialization failed"); vTaskDelete(nullptr); }
    ...
    ESP_LOGI("LATHE_UI", "READY: ESP32-P4 lathe touchscreen on grblHAL; 1280x800; eight operation tabs");
    lathe_audio_start();
    ui_ready.store(true);
    for (;;) {
        bsp_display_lock(portMAX_DELAY);
        ...
        while (xQueueReceive(test_actions, &action, 0) == pdTRUE) {
            ...
            if (action=='6' && !lathe_cycle_busy() && !status.moving) {
            ...
            if(strchr("FCEGKA",action) && !lathe_cycle_busy() && !status.moving) {
            ...
            if(action=='R' && !lathe_cycle_busy() && !status.moving) {
            ...
            if(action=='U') {lathe_ui_show_update();continue;}
...
extern "C" void lathe_ui_start(void)
{
    int32_t limits[4];
    lathe_saved_limits_get(limits);
    ...
    uint8_t disabled = lathe_saved_disabled_get();
    x.disabled = disabled & 1;
    z.disabled = disabled & 4;
    lathe_preferences_t prefs;
    if(lathe_preferences_get(&prefs)) {
    ...
    configASSERT(xTaskCreatePinnedToCore(ui_task, "LATHE_UI", 16384, nullptr, 2, nullptr, 0) == pdPASS);
}

extern "C" bool lathe_ui_ready(void) { return ui_ready.load() && lathe_audio_initialized() && lathe_network_initialized(); }
extern "C" uint32_t lathe_ui_updates(void) { return ui_updates.load(std::memory_order_relaxed); }
extern "C" bool lathe_ui_test_action(char action)
{
    return ui_ready.load() && (!p4_motor_controls_enabled() || strchr("VBWUQ",action)) && strchr("123405678FCEGKAUDQR+NVBW", action) && xQueueSend(test_actions, &action, 0) == pdTRUE;
}
extern "C" bool lathe_ui_screenshot(void (*write)(const char *))
{
