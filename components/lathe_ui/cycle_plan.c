...
bool lathe_cycle_plan(const lathe_cycle_config_t *c, const lathe_cycle_machine_t *m, lathe_cycle_plan_t *p, char *error,
                   size_t size)
{
    memset(p, 0, sizeof(*p));
    ...
    if (c->operation > LATHE_ELLIPSE)
        return fail(error, size, "Unknown cycle operation");
    if (c->x_min >= c->x_max || (c->operation != LATHE_CUT && c->z_min >= c->z_max))
    ...
    p->config = *c;
    if (c->threading)
        p->config.operation = LATHE_THREAD;
    bool face = p->config.operation == LATHE_FACE, cut = p->config.operation == LATHE_CUT;
    p->cut_axis = face || cut ? 'X' : 'Z';
    p->depth_axis = face || cut ? 'Z' : 'X';
    // Thread needs phase registration. Ordinary turning uses G95 like facing.
    p->indexed = p->config.operation == LATHE_THREAD;
    ...
    p->starts = p->config.operation == LATHE_THREAD ? c->starts : 1;
    ...
    if (p->config.operation == LATHE_ELLIPSE) {
        // Preserve the controller's spindle-progress parameter: each pass is a scaled
...
double lathe_cycle_depth(const lathe_cycle_plan_t *p, unsigned pass)
{
    return p->depth_start + (p->depth_end - p->depth_start) * (pass + 1) / p->config.passes;
}
unsigned lathe_cycle_phase(const lathe_cycle_plan_t *p, unsigned start)
{
    return lathe_cycle_phase_at(p, start, p->approach, p->spindle_direction);
}
unsigned lathe_cycle_phase_at(const lathe_cycle_plan_t *p, unsigned start, double position, int spindle_direction)
{
...
const char *lathe_cycle_name(lathe_cycle_operation_t op)
{
    static const char *names[] = {"Turn", "Thread", "Face", "Cut", "Ellipse"};
    return op <= LATHE_ELLIPSE ? names[op] : "Unknown";
}
unsigned lathe_cycle_segment_at(const lathe_cycle_plan_t *p, unsigned pass, double x, double z)
{
    // Locate the stopped point, not the last queued chord: lookahead can be far ahead.
    unsigned nearest = 0;
    double best = INFINITY, ax, az, feed;
    lathe_cycle_point(p, pass, 0, &ax, &az, &feed);
    for (unsigned i = 0; i < p->segments; i++) {
        double bx, bz;
        lathe_cycle_point(p, pass, i + 1, &bx, &bz, &feed);
...
void lathe_cycle_point(const lathe_cycle_plan_t *p, unsigned pass, unsigned segment, double *x, double *z,
                    double *feed)
{
