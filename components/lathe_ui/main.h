...
double lathe_ui_work_offset(Axis *axis);
bool lathe_ui_set_jog_limits(bool enabled);
double lathe_ui_limit_coordinate(Axis *axis, long steps);
bool lathe_ui_limit_steps(Axis *axis, double coordinate, long *steps);
const char *lathe_ui_apply_limits(const long limits[4]); // X-, X+, Z-, Z+; nullptr on success.
