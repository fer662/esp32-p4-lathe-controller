...
LimitEditor::LimitEditor(lv_obj_t *parent) {
  ...
  keypad = std::make_unique<Numpad>(panel, [this](float value, Numpad::Action action) {
    ...
    long converted;
    if (!coordinatesUnchanged() || !lathe_ui_limit_steps(axisFor(i), value, &converted)) {
...
void LimitEditor::show() {
  ...
  lathe_ui_limits_editable(); // Refresh the authoritative core coordinate snapshot.
  editMeasure = measure;
  editOffset = {lathe_ui_work_offset(&x), lathe_ui_work_offset(&z)};
  editSystem = lathe_ui_work_system();
  keypad->hide();
  refresh();
...
void LimitEditor::refresh() {
  ...
  for (unsigned i = 0; i < draft.size(); ++i) {
    ...
    value += draft[i] == unset(i) ? "Unset" : format_decimal(lathe_ui_limit_coordinate(axisFor(i), draft[i]), 3) + " " + unit;
  ...
  for (unsigned i = 0; i < 2; ++i) {
    ...
    else value += format_decimal(lathe_ui_limit_coordinate(axisFor(low), draft[high]) -
                                 lathe_ui_limit_coordinate(axisFor(low), draft[low]), 3) + " " + unit;
...
bool LimitEditor::coordinatesUnchanged() {
  lathe_ui_limits_editable();
  return measure == editMeasure && editSystem == lathe_ui_work_system() &&
         editOffset[0] == lathe_ui_work_offset(&x) && editOffset[1] == lathe_ui_work_offset(&z);
}
void LimitEditor::apply() {
  if (!coordinatesUnchanged()) {
    lv_label_set_text(message, "Work zero or units changed. Cancel and reopen the editor."); return;
  }
  if (const char *error = lathe_ui_apply_limits(draft.data())) { lv_label_set_text(message, error); return; }
  lv_obj_add_flag(panel, LV_OBJ_FLAG_HIDDEN);
}
