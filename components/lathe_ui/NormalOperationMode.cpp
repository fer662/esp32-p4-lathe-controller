...
void NormalOperationMode::createPassesButton() {
  ...
  LVCallbackWrapper::add(passesButton, LV_EVENT_CLICKED, [this](lv_event_t *e) {
    if (!lathe_cycle_busy()) this->numpad->show(Numpad::PASSES_SETTING);
    Buzzer::getInstance().beepSuccess();
  });

  // Initialize the button appearance
  LVCallbackWrapper::add(passesButton, LV_EVENT_LONG_PRESSED, [](lv_event_t *) { lathe_cycle_advance(); });
...
void NormalOperationMode::createLimitControls() {
  ...
  LVCallbackWrapper::add(jogLimitsButton, LV_EVENT_CLICKED, [this](lv_event_t *) {
    if (lathe_ui_set_jog_limits(!jogLimitsEnabled)) {
...
void NormalOperationMode::dpadButtonDownCallback(DPad::Direction id,
  ...
  if (self->shiftButtonState && !isOn) {
    ...
    switch (id) {
    case DPad::BTN_UP:
      lathe_ui_jog(&x, 1, true);
      break;
    case DPad::BTN_DOWN:
      lathe_ui_jog(&x, -1, true);
      break;
    case DPad::BTN_LEFT:
      lathe_ui_jog(&z, 1, true);
      break;
    case DPad::BTN_RIGHT:
      lathe_ui_jog(&z, -1, true);
      break;
...
void NormalOperationMode::dpadButtonUpCallback(DPad::Direction id,
  ...
  switch (id) {
  case DPad::BTN_UP:
    lathe_ui_jog(&x, 1, false);
    break;
  case DPad::BTN_DOWN:
    lathe_ui_jog(&x, -1, false);
    break;
  case DPad::BTN_LEFT:
    lathe_ui_jog(&z, 1, false);
    break;
  case DPad::BTN_RIGHT:
    lathe_ui_jog(&z, -1, false);
    break;
