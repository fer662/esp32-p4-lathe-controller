...
try:
    ...
    cuts=[s for s in lines if s.startswith('[P4DONE:') and '|STAGE:Cut|' in s]
    ...
    for action in 'GKA':
        ...
        reference=next(s for s in reversed(b.history) if s.startswith('[P4FOLLOWREF:'))
        expected=2*float(reference.split('|PITCH:')[1].rstrip(']'))
        time.sleep(.3);b.command('$P4UITEST=+')
        b.receive(lambda s:s.startswith('[P4FOLLOWREF:') and f'|PITCH:{expected:.6f}' in s,10)
        b.receive(lambda s:'STAGE:Following' in s,10)
